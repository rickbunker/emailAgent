...
class SenderMappingResponse(BaseModel):
    ...
    created_date: str
    last_updated: str
    # active: bool  # Not implemented in current model
...
async def sender_mapping_to_response(
    ...
    if isinstance(last_activity, str):
        ...
        last_activity = last_activity.isoformat() if last_activity else created_date

    return SenderMappingResponse(
        sender_email=mapping.sender_email,
        default_asset_id=mapping.asset_id,
        sender_name=metadata.get("sender_name"),
        organization=metadata.get("organization"),
        notes=metadata.get("notes"),
        created_date=created_date,
        last_updated=last_activity,
        # active=True,  # All mappings are active
...
async def list_sender_mappings(
    ...
    """
    try:
        # Get all mappings (returns tuple of mappings and count)
        all_mappings, total_count = await sender_service.list_all_mappings(limit=1000)

        # Apply filters
        filtered_mappings = all_mappings

        # Note: 'active' field not implemented in current SenderMapping model
        # All mappings are considered active
        ...
        if search:
            ...
            filtered_mappings = [
                ...
                or (
                    m.metadata
                    and (
                        search_lower in m.metadata.get("organization", "").lower()
                        or search_lower in m.metadata.get("notes", "").lower()
                    )
                )
            ]
            ...
            by_asset[asset_id]["senders"].append(
                {
                    ...
                    "name": (
                        mapping.metadata.get("sender_name")
                        if mapping.metadata
                        else None
                    ),
                    "organization": (
                        mapping.metadata.get("organization")
                        if mapping.metadata
                        else None
                    ),
                }
