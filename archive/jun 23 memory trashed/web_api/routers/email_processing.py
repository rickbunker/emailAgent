...
async def process_emails(
    ...
    try:
        ...
        for email in emails:
            ...
            # If email was found by has_attachments=True but has no attachments loaded,
            # reload it with explicit attachment inclusion
            if len(email.attachments) == 0:
                logger.info(f"Reloading email {email.id} with attachment details")
                try:
                    email = await email_interface.get_email(
                        email.id, include_attachments=True
                    )
                    logger.info(
                        f"Reloaded email has {len(email.attachments)} attachments"
                    )
                except Exception as e:
                    logger.warning(
                        f"Failed to reload email {email.id} with attachments: {e}"
                    )
                    # Continue with original email object
    ...
    try:
        ...
        for attachment in email.attachments:
            try:
                ...
                # Get attachment content - download separately if not already loaded
                content = attachment.content
                if not content and attachment.attachment_id:
                    logger.info(
                        f"Downloading attachment content for: {attachment.filename}"
                    )
                    try:
                        content = await email_interface.download_attachment(
                            email.id, attachment.attachment_id
                        )
                        # Update the attachment object with downloaded content
                        attachment.content = content
                        logger.info(
                            f"Downloaded {len(content)} bytes for {attachment.filename}"
                        )
                    except Exception as download_error:
                        logger.error(
                            f"Failed to download attachment {attachment.filename}: {download_error}"
                        )
                        continue

                if not content:
                    logger.warning(
                        f"No content available for attachment: {attachment.filename}"
                    )
                    continue
