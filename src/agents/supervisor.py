...
class EmailAnalysis:
    ...
    def __post_init__(self) -> None:
        ...
        if not 0.0 <= self.priority_score <= 1.0:
            raise ValueError(
                f"Priority score must be 0.0-1.0, got {self.priority_score}"
            )
...
class EmailSupervisor:
    ...
    def __init__(
        self, spam_threshold: float = 0.7, priority_threshold: float = 0.6
    ) -> None:
        """
    ...
    async def process_email(self, email: EmailMessage) -> EmailAnalysis:
        ...
        try:
            ...
            confidence = self._calculate_confidence(
                sender_info, spam_score, priority_score
            )
    ...
    async def _analyze_sender(self, sender_email: str) -> dict[str, Any]:
        ...
        try:
            ...
            search_results = await self.semantic_memory.search(
                query=f"sender:{sender_email}", limit=3
            )
            ...
            for memory_item, score in search_results:
                if score > 0.8:  # High confidence match
                    ...
                    self.logger.debug(
                        f"Found sender data: trust={trust_str}, type={sender_type}"
                    )
            ...
            for memory_item, score in domain_results:
                if score > 0.7:
                    ...
                    self.logger.debug(
                        f"Found domain pattern: trust={domain_trust}, type={domain_type}"
                    )
    ...
    @log_function()
    async def _analyze_spam(
        self, email: EmailMessage, sender_info: dict[str, Any]
    ) -> float:
        """
        ...
        try:
            ...
            spam_rules = await self.procedural_memory.search(
                query="spam detection rule", limit=10
            )
            ...
            for memory_item, relevance in spam_rules:
                if relevance > 0.7:  # High relevance
                    ...
                    if rule_type == "attachment_suspicious":
                        ...
                        if "urgent" in content_lower and any(
                            word in content_lower
                            for word in ["money", "payment", "transfer", "wire"]
                        ):
                            spam_score += 0.6
            ...
            bulk_count = sum(
                1 for indicator in bulk_indicators if indicator in content_lower
            )
            if bulk_count > 1:
                spam_score += 0.3
    ...
    async def _analyze_priority(
        ...
        try:
            ...
            if email.received_date:
                hours_old = (
                    datetime.now(UTC) - email.received_date
                ).total_seconds() / 3600
    ...
    async def _generate_recommendations(
        ...
        try:
            ...
            # If not spam, proceed with other actions
                ...
                if email.attachments:
                    ...
                    if has_docs:
                        ...
                        reasoning_parts.append(
                            "Document attachments suggest asset materials"
                        )
                ...
                if trust_level in [TrustLevel.HIGH, TrustLevel.HIGHEST] and any(
                    trigger in content_lower for trigger in auto_response_triggers
                ):
                ...
                if (
                    trust_level in [TrustLevel.MEDIUM, TrustLevel.HIGH]
                    and priority_score < 0.2
                ):
            ...
            self.logger.debug(
                f"Generated {len(actions)} actions: {[a.value for a in actions]}"
            )
            ...
            return [
                EmailAction.REQUIRE_ATTENTION
            ], f"Recommendation generation failed: {e}"

    @log_function()
    def _calculate_confidence(
        self, sender_info: dict[str, Any], spam_score: float, priority_score: float
    ) -> float:
        """
    ...
    async def _store_analysis(
        self, email: EmailMessage, analysis: EmailAnalysis
    ) -> None:
        """
    ...
    async def provide_feedback(
        self, email_id: str, feedback_type: str, feedback_data: dict[str, Any]
    ) -> None:
        """
        ...
        try:
            self.logger.info(
                f"Processing feedback for email {email_id}: {feedback_type}"
            )
    ...
    async def get_processing_statistics(self) -> dict[str, Any]:
        ...
        try:
            ...
            recent_analyses = await self.episodic_memory.search(
                query="email analysis", limit=100
            )
            ...
            for memory_item, score in recent_analyses:
                if score > 0.7:  # High relevance
                    ...
                    stats["trust_distribution"][trust] = (
                        stats["trust_distribution"].get(trust, 0) + 1
                    )
            ...
            self.logger.info(
                f"Processing statistics: {stats['total_processed']} emails processed"
            )
            return stats
