...
def create_memory_system(memory_type: str, **kwargs) -> BaseMemory:
    ...
    if memory_type == "episodic":
        ...
        raise ValueError(
            f"Unsupported memory type: '{memory_type}'. Supported types: {supported_types}"
        )
...
async def create_unified_memory_system(**kwargs) -> dict:
    ...
    try:
        ...
        logger.info(
            f"Successfully created unified memory system with {len(memory_systems)} memory types"
        )
...
class MemorySystemFactory:
    ...
    def create_optimized_memory_config(
        total_memory_items: int = 10000, workload_type: str = "balanced"
    ) -> dict:
        """
        ...
        """
        logger.info(
            f"Creating optimized memory config: {workload_type}, {total_memory_items} items"
        )
